...
**`ASCLL`编码表**：采用8位二进制数来对应一个英文字符（只支持英文字符串）。

**`GBK`编码表**：采用16位二进制数来对应一个中英文字符。

1. 支持中英文字符
2. 采用8位二进制数对应一个英文字符
3. 采用16位二进制数对应一个中文字符

**`unicode`编码表**:现代计算机采用该编码。

1. 兼容万国字符
2. 采用16位（16bit=2Bytes）二进制对应一个中文字符,个别生僻字符会采用4Bytes、8Bytes。
3. 各国之前制定的字符编码与万国字符编码都有对应关系，来进行编码转换。（解决历史遗留问题）
...
> 注意：内存固定使用`unicode`，因此只能改变存入硬盘采用格式来进行转换。

### 编码乱码问题

存乱：编码格式应该设置成支持文件内存字符的格式。

取乱：文件是以什么编码格式存入硬盘的，应该用对应的编码格式存入内存。

### 编码与解码

**编码图解：**

![bm](../.vuepress/public/bm.jpg)

**解码图解：**

![jm](../.vuepress/public/jm.jpg)

### 使用文件操作

- **控制文件读写内容的模式**

> 注意：t和b不能单独使用，必须与/r/w/a连用。

1. t.文本（默认模式）
    - 读写都是以`str`(unicode)为单位
    - 必须都是文本文件
    - 必须指定encoding = "utf-8"
2. b.二进制/bytes
...
- 当文件不存在时，会创建空文件。
- 当文件存在时，w模式会清空文件，指针位于开始位置。

> 注意：在w模式下（没有关闭的情况下），连续写入新的内容，总是会在旧的内容之后写入。
